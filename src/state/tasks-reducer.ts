...
export const createTaskTC = (task: { addedDate: string; todoListId: any; description: string; id: string | Uint8Array; title: string; priority: TaskPriorities; deadline: string; startDate: string; status: TaskStatuses; order: number }): AppThunkType => {
    return (dispatch) => {
        dispatch(appSetStatusAC('loading'));
